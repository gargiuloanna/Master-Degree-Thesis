...
from plotting.plots import confusion_matrix, random_forest_fimp, permutation_imp, feature_importance
from preprocessing.scale import scale
from  sklearn.metrics import balanced_accuracy_score
import os

if __name__ == '__main__':
    name = "RandomForestGrid-Test25%-CV5-OOB-scaledata-class-weight"
    ...
    rdf_gridcv = GridSearchCV(rdf_model, param_grid=param_grid, cv=5, scoring='balanced_accuracy',  error_score=0, n_jobs=-1, verbose=3, refit=True)
    ...
    rdf = dump(rdf_gridcv.best_estimator_, "C:/Users/annin/PycharmProjects/Tesi/Models/" + name + ".joblib")
    print("[RANDOM FOREST] Model Saved")

    print("Test Accuracy: ", predict_score(rdf_gridcv.best_estimator_, test, labeltest))
    file = open("performances.txt", "a")
    file.write("Model " + name + '\n')
    file.write("Train Accuracy: " + str(clf.oob_score_) + '\n')
